		...
		foreach ($results as $form_result) {
			...
			if ($form_result->type == 'form') {
				if (in_array('captcha', $form_result->parameters)) {
					...
					'<div id="form_' . $formname . '_builder_captcha" class="form_info cmsms-form-builder-captcha">' . recaptcha_get_html($cmsms_option[CMSMS_SHORTNAME . '_recaptcha_public_key']) . '</div>' . "\n";
				}
				...
				'<div class="form_info submit_wrap">' . "\n" . 
					'<a id="cmsms_' . $formname . '_formsend" class="button" href="#"><span>' . $form_but[2] . '</span></a>' . "\n";
				
				if (in_array('reset', $form_result->parameters)) {
					$out .= '<a style="margin:0 0 0 10px;" id="cmsms_' . $formname . '_formclear" class="button" href="#" onclick="if (confirm(\'' . __('Do you really want to reset the form?', 'cmsms_form_builder') . '\')) document.getElementById(\'form_' . $formname . '\').reset(); return false;"><span>' . __('Reset', 'cmsms_form_builder') . '</span></a>' . "\n";
				}
