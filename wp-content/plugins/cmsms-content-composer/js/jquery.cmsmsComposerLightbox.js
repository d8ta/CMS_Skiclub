	...
	var ComposerLightbox = function (element, parameters) { 
		...
		obj.methods = { 
			...
			setVars : function () { 
				...
				obj.methods.themeURL = $('#cmsms_composer_url').val();
			...
			generateField : function (key, field, val, shcd) { 
				...
				case 'editor':
					...
					var editorStartInterval = setInterval(function () { 
						if (obj.methods.startEditor) {
							privateMethods.doEditor(key + '_' + obj.methods.uniqID);
					...
					break;
				case 'input':
					fieldContent += '<input type="text" id="' + key + '_' + obj.methods.uniqID + '" name="' + key + '_' + obj.methods.uniqID + '" value="' + privateMethods.unSanitizeContent(val) + '"' + ((field.required) ? ' aria-required="true"' : '') + ((field.min) ? ' min="' + field.min + '"' : '') + ((field.max) ? ' max="' + field.max + '"' : '') + ((field.step) ? ' step="' + field.step + '"' : '') + ' />';
					
					
					if (field.width === 'number') {
						obj.methods.eventsArray.push('number');
					}
				...
				case 'icon':
					fieldContent += '<div class="icons_list_parent">' + 
						'<a href="#" class="cmsms_icon_cancel admin-icon-remove"' + ((val !== '') ? '' : ' style="display:none;"') + '>' + cmsms_lightbox.deselect + '</a>' + 
						'<div class="cmsms_icon_search">' + 
							'<label>' + cmsms_lightbox.find_icons + ':</label>' + 
							'<input type="text" name="cmsms_icon_search" />' + 
						'</div>' + 
						'<div class="cl" />';
					
					
					for (var font in field.choises) {
						fieldContent += '<h3>' + font.slice(0, 1).toUpperCase() + font.slice(1) + '</h3>' + 
						'<ul>';
						
						
						for (var k in field.choises[font]) {
							fieldContent += '<li' + ((field.choises[font][k] === val) ? ' class="active"' : '') + '><a class="' + field.choises[font][k] + '" data-code="' + k + '" title="' + k + '" /></li>';
						}
					...
					fieldContent += '<input type="hidden" id="' + key + '_' + obj.methods.uniqID + '" name="' + key + '_' + obj.methods.uniqID + '" value="' + val + '" />'  + 
					'</div>';
					
					
					obj.methods.eventsArray.push('icon');
				...
				case 'table':
					...
					for (var i = 0, ilength = tr.length; i < ilength; i += 1) {
						...
						for (var j = 0, jlength = td.length; j < jlength; j += 1) {
							...
							fieldContent += '<div class="cmsms_table_cell cmsms_change_cell">' + td[j] + '</div>';
						}
		...
		privateMethods = { 
			...
			doEditor : function (id) { 
				privateMethods.generateEditor(id);
				
				
				setTimeout(function () { 
					privateMethods.editorToolbarsTrigger(id);
				}, 300);
			}, 
			generateEditor : function (editorID) { 
				...
				containerWrap.find('.switch-tmce').bind('click', function () { 
					...
					privateMethods.editorToolbarsTrigger(editorID);
				} );
			...
			editorToolbarsTrigger : function () { 
				if ($('.mceEditor.wp_themeSkin .mceToolbar.mceToolbarRow2:eq(-1)').css('display') !== 'none') {
					$('.mceEditor.wp_themeSkin .mceToolbar.mceToolbarRow3:eq(-1)').show();
					
					
					$('.mceEditor.wp_themeSkin a.mce_wp_adv:eq(-1)').addClass('mceButtonActive');
				}
			}, 
			...
			loadMultiple : function (data, content, id) { 
				...
				console.log(data.replace(/<\/p>\s+\[\//g, '</p>[/').replace(/\]\s+<p>/g, ']<p>').replace(/<\/p>\s+<([^\/])/g, '</p><$1'));
			...
			attachGeneratedEvents : function () { 
				...
				// Type Number
				if ($.inArray('number', obj.methods.eventsArray) !== -1) {
					// Number Spinner
					$('#cmsmsBox_' + obj.methods.uniqID + ' .number_width input[type="text"]').spinner( { 
				...
				// Type Icon
				if ($.inArray('icon', obj.methods.eventsArray) !== -1) {
					// Icons Filter
					$('#cmsmsBox_' + obj.methods.uniqID + ' .cmsms_icon_search > input[type="text"]').bind('input', function () { 
						var val = $(this).val();
					...
					// Icon Choose
					$('#cmsmsBox_' + obj.methods.uniqID + ' .icons_list_parent > ul > li > a').bind('click', function () { 
						var parentLi = $(this).parent(), 
							li = $(this).parents('.icons_list_parent').find('> ul > li'), 
							cancel = $(this).parents('.icons_list_parent').find('> a.cmsms_icon_cancel'), 
							hidden = $(this).parents('.icons_list_parent').find('#icon_' + obj.methods.uniqID);
					...
					// Icon Cancel
					$('#cmsmsBox_' + obj.methods.uniqID + ' .icons_list_parent > a.cmsms_icon_cancel').bind('click', function () { 
			...
			getURL : function () { 
				return obj.methods.themeURL + '/framework/admin/composer/inc/composer_popup.php';
			}, 
			getUniqID : function () { 
